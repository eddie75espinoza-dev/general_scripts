...
For example, given the string s=lmnop, the ordinal values of the charcters are [108,109,110,111,112] . s(reverse)= ponml
and the ordinals are [112,111,110,109,108]. The absolute differences of the adjacent elements for both strings are , so
the answer is Funny.
...
def funnyString(s):
    ...
    for i in range(0,len(res1)-1):
        b1 = abs( res1[i] - res1[i+1] )
        b.append(b1)
    c = []
    for i in range(0, len(res2)-1):
        c1 = abs (res2[i] - res2[i+1])
        c.append(c1)
...
if __name__ == '__main__':
    ...
    q = int(input('cantidad de interacciones: '))
